  ...
  return (
    <div style={{ marginBottom: '12px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
        ...
        <span style={{ color, fontSize: '13px', fontWeight: '600' }}>{Math.round(value)}g <span style={{ color: '#4a6080', fontWeight: '400' }}>/ {target}g</span></span>
      </div>
...
export default function Nutrition({ user }) {
  ...
  const targets = bodyweight ? {
    calories: Math.round(bodyweight * 15),
    protein: Math.round(bodyweight * 0.8),
    carbs: Math.round((bodyweight * 15 * 0.45) / 4),
    ...
    <div className="screen">
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
        {[
          ...
          <button key={tab.key} onClick={() => { setView(tab.key); setScannedResult(null) }} style={{ flex: 1, padding: '10px', borderRadius: '10px', border: 'none', cursor: 'pointer', fontWeight: '600', fontSize: '13px', backgroundColor: view === tab.key ? '#0ea5e9' : '#0d1520', color: view === tab.key ? 'white' : '#4a6080', transition: 'all 0.2s' }}>{tab.label}</button>
        ))}
      </div>
      ...
      {view === 'summary' && (
        <div>
          <div style={{ background: '#0d1520', borderRadius: '16px', padding: '20px', marginBottom: '12px', border: '0.5px solid #1e2a3a' }}>
            ...
            {bodyweight ? (
              <p style={{ color: '#4a6080', fontSize: '12px', margin: '0 0 16px' }}>Based on your bodyweight of {bodyweight} lbs</p>
            ) : (
              <p style={{ color: '#f59e0b', fontSize: '12px', margin: '0 0 16px' }}>Log your bodyweight in check-in for personalized targets</p>
            )}
          ...
          <div style={{ background: '#0d1520', borderRadius: '16px', padding: '20px', marginBottom: '12px', border: '0.5px solid #1e2a3a' }}>
            ...
            {[
              { label: 'Fiber', value: `${Math.round(totalNutrition.fiber)}g`, target: '25-35g', color: '#2ecc71' },
              { label: 'Sugar', value: `${Math.round(totalNutrition.sugar)}g`, target: '<50g', color: '#9b59b6' },
              { label: 'Sodium', value: `${Math.round(totalNutrition.sodium)}mg`, target: '<2300mg', color: '#e74c3c' },
            ].map((item, i) => (
