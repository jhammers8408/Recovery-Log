...
import { registerServiceWorker, checkStreakAndNotify } from './notifications'
...
<p style={{ color: '#4a6080', fontSize: '14px' }}>What actually improves YOUR performance.</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
          {[{ label: 'Apple', icon: '🍎' }, { label: 'Google', icon: '🔵' }].map(btn => (
            <div key={btn.label} style={{ backgroundColor: '#111820', border: '1px solid #1e2a3a', borderRadius: '12px', padding: '13px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', cursor: 'pointer' }}>
              <span style={{ fontSize: '16px' }}>{btn.icon}</span>
              <span style={{ color: '#8aa0b8', fontSize: '14px', fontWeight: '600' }}>{btn.label}</span>
            </div>
          ))}
        </div>
      </div>
